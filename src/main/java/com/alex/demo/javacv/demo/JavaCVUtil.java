...
public class JavaCVUtil {
	 ...
	 */
	public static FFmpegFrameGrabber initGrabber(String inputPath, int frameRate, int iamgeWidth, int imageHeight, int videoCodec,
			boolean exterCamera) throws FrameGrabber.Exception {
	 ...
	 */
	public static FrameRecorder initRecorder(String ouputPath, int frameRate, String format, int width, int heigth, int videoCodec, int audioCodec,
			int audioChannels, int pixelFormat) throws FrameRecorder.Exception {
		FrameRecorder recorder = FrameRecorder.createDefault(ouputPath, width, heigth);
		recorder.setVideoCodec(videoCodec); // avcodec.AV_CODEC_ID_H264，编码
		recorder.setAudioCodec(audioCodec);
		recorder.setAudioChannels(audioChannels);
		recorder.setFormat(format);
		recorder.setFrameRate(frameRate);
		recorder.setPixelFormat(pixelFormat);// 设置像素
		return recorder;
	}
